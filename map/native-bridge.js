/* Udaan native bridge
   ...
   styling, all wired to the same UI Udaan already has. */
(function () {
  ...
  window.udaanShare = function (res) {
    ...
    const text = `Udaan check for ${lat}, ${lon}: ${label}`;

    if (isNative && Cap.Plugins && Cap.Plugins.Share) {
      Cap.Plugins.Share.share({ title: 'Udaan — drone airspace check', text, url, dialogTitle: 'Share this spot' }).catch(() => {});
      return;
    }
    if (navigator.share) {
      navigator.share({ title: 'Udaan', text, url }).catch(() => {});
      return;
    }
