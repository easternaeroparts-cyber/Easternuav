...
/* This app's own base URL, worked out from where it is actually served.
   Hard-coding the folder here is what breaks auth redirects whenever the
   path changes, so don't. */
    ...
    try {
      if (loginMode === 'reset') {
        ...
        const { error } = await sb.auth.signUp({
          email, password: pass,
          options: { data: { full_name: $('#lg-name').value.trim() }, emailRedirectTo: APP_URL }
        });
        if (error) throw error;
        msg.style.color = 'var(--green)';
